...
export class QuantitySelectorComponent extends Component {
  ...
  updateConstraints(min, max, step) {
    ...
    const newMin = Number.isNaN(parsedMin) ? 1 : parsedMin; // allow 0
    const parsedStep = parseInt(step);
    const newStep = Number.isNaN(parsedStep) ? 1 : parsedStep;
  ...
  getCurrentValues() {
    const { quantityInput } = this.refs;
    const parsedMinCurrent = parseInt(quantityInput.min);
    const parsedStepCurrent = parseInt(quantityInput.step);
    return {
      min: Number.isNaN(parsedMinCurrent) ? 1 : parsedMinCurrent, // allow 0
      max: quantityInput.max ? parseInt(quantityInput.max) : null,
      step: Number.isNaN(parsedStepCurrent) ? 1 : parsedStepCurrent,
